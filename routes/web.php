...
Route::get('/', function (Request $request) {
    ...
    if($user && $viewAs!=='anonymous' && $request->query('error')==null){
...
Route::get('/reset-password/{token}', function (string $token) {
    ...
    return view('login.reset-password', ['token' => $token, 'email' => $email, 'avatars'=>Avatar::all()]);
...
})->name('editable_field')->middleware('auth:sanctum');
...
Route::get('/admin/view-as/{type}', function ($type) {
    ...
    return redirect('/dashboard');
})->middleware('auth:sanctum')->name('admin.view-as');
